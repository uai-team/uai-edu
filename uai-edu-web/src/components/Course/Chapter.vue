  ...
  <div class="sy_body">
    <div v-for="(one, index) in list" :key="index" class="chapter_info">
      <div class="chapter_name">
        <span>{{ index + 1 }}.&nbsp;&nbsp;</span>{{ one.chapter_name }}
      </div>
      <div v-for="(two, num) in one.lessonRespList" :key="num" class="period_info">
...
<script setup>
  // import { formatTime, getResourceTypeName } from '@/utils/base'
  ...
  function handlePlayVideo(lesson) {
    // 资源
    router.push('/course/study?id=' + route.query.id + '&lessonId=' + lesson.id)
  }
</script>
