<template>
  <div class="simulation-detail">
    <Affix ref="affixRef" :showEditor="showEditor" :imageUrl="imageUrl" />
    <div class="simulation-header">
      <div class="header-left">
        <span class="cursor" @click="handleBack">
          <img class="cursor-image" src="@/assets/svg/return.svg" alt="return" />
        </span>
        <router-link :to="{ path: '/simulation/detail', query: { id: simulationInfo?.id } }" class="left_col">
    ...
    <div class="simulation-body">
      <div class="simulation-content">
        <iframe :src="simulationInfo.simulation_path" width="100%" height="100%" frameborder="0"></iframe>
      </div>
...
const onSend = async function (message) {
  ...
  const user = {
    ...
    image: imagefile.value.length > 0 ? imageBase64.value : '',
    content: message,
    role: 'user',
  };
  ...
  await client.chat.completions.create({
    ...
    messages: user.image ? [
      ...
      { "role": "system", "content": simulationInfo.value?.simulation_prompt || "你是一名资深的教育专家。" },
...
const messageListener = (event) => {
  if (event.data.course && event.data.messages) {
    const chatConfig = AIChat.models[selectValue.value.value];
    const client = new OpenAI({
      baseURL: chatConfig.baseUrl,
      apiKey: chatConfig.apiKey,
      dangerouslyAllowBrowser: true,
  ...
  } else if (event.data.screenshot && event.data.type === 'course-ask') {
    imagefile.value = [{ url: event.data.screenshot }];
  } else if (event.data.screenshot && event.data.type === 'course-note') {
    showEditor.value = true
    imageUrl.value = event.data.screenshot
    // affixRef.value.insertImage(event.data.screenshot);
  }
}
