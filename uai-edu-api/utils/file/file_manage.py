...
class FileManage(FileBase):
    ...
    def __init__(self, file: UploadFile, path: str, mode: str = "generate"):
        self.mode = mode
        if mode == "generate":
        ...
        if mode == "custom":
            ...
            if path[-1] == "/":
                path = path[:-1]
            self.path = f"{STATIC_ROOT}/{path}/{file.filename}"
        self.file = file

    async def save_file_local_path(self, accept: list = None) -> dict:
