...
class DeptDal(DalBase):
    ...
    async def get_dept_children_id(
        self,
        id: int, 
    ) -> list:
        sql = select(self.model)
        ...
        depts = self.generate_children_id(datas, roots)
        if id:
            depts.append(id)
        return depts


    def generate_children_id(self, depts: list[models.VadminDept], nodes: filter) -> list:
        data = []
        for root in nodes:
            router = schemas.DeptTreeListOut.model_validate(root)
            sons = filter(lambda i: i.parent_id == root.id, depts)
            children = self.generate_children_id(depts, sons)
    ...
    async def get_dept_parent_id(
        ...
        queryset = await self.db.scalars(sql)
        datas = list(queryset.all())
        nodes = filter(lambda i: i.id == id, datas)
        depts = self.generate_parent_id(datas, nodes)
        if id:
            depts.append(id)
        return depts


    def generate_parent_id(self, depts: list[models.VadminDept], nodes: filter) -> list:
        data = []
        for child in nodes:
            router = schemas.DeptTreeListOut.model_validate(child)
            fathers = filter(lambda i: i.id == child.parent_id, depts)
            parents = self.generate_parent_id(depts, fathers)
            data.append(router.id)
...
class TestDal(DalBase):
    ...
    async def relationship_where_operations_has(self):
        ...
        # 查找出只有满足关联了用户名称为 "admin" 的所有帮助问题，没有关联的则不会查询出来
        sql1 = select(vadmin_help_models.VadminIssue).where(
            vadmin_help_models.VadminIssue.create_user.has(models.VadminUser.name == "admin")
        )
        queryset1 = await self.db.scalars(sql1)
        result1 = queryset1.unique().all()
        for data in result1:
